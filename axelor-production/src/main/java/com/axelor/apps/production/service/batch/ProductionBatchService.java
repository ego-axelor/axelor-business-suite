...
public class ProductionBatchService extends AbstractBatchService {
  ...
  public Batch run(Model batchModel) throws AxelorException {
    ...
    Batch batch;
    ProductionBatch productionBatch = (ProductionBatch) batchModel;

    switch (productionBatch.getActionSelect()) {
      case ProductionBatchRepository.ACTION_PLAN:
        batch = plan(productionBatch);
        break;
      default:
  ...
  private Batch plan(ProductionBatch productionBatch) throws AxelorException {
    return Beans.get(BatchManufOrderPlan.class).run(productionBatch);
  }
}
