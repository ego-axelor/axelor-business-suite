...
public class ManufOrderServiceImpl implements ManufOrderService {
  ...
  protected BigDecimal computeDiffQty(
	...
	public void optaPlan(List<ManufOrder> manufOrderList) throws AxelorException {
		...
		// Build the Solver
		SolverFactory<Schedule> solverFactory = SolverFactory
				.createFromXmlResource("projectjobscheduling/solver/projectJobSchedulingSolverConfig.xml");
		...
		for (WorkCenter workCenter : workCenterList) {
			...
			long resourceId = unsolvedJobScheduling.getResourceList().size() > 0
					? unsolvedJobScheduling.getResourceList().get(unsolvedJobScheduling.getResourceList().size() - 1)
							.getId() + 1
					: 0;
		...
		for (ManufOrder manufOrder : manufOrderList) {
			// Create project
			Project project = createProject(unsolvedJobScheduling, manufOrder, machineCodeToResourceMap,
		...
		for (Allocation allocation : solvedJobScheduling.getAllocationList()) {
			...
			if (prodProcessLine != null) {
				...
				operationOrder.setPlannedDuration(DurationTool.getSecondsDuration(
		...
		for (Allocation allo : solvedJobScheduling.getAllocationList()) {
			...
			for (int i = allo.getStartDate(); i < allo.getEndDate(); i++)
				dashes += "#";
			System.out.println(StringUtils.rightPad(allo + " Machine : "
					+ (allocationIdToProdProcessLineMap.get(allo.getId()) != null
							? allocationIdToProdProcessLineMap.get(allo.getId()).getWorkCenter().getName()
							: "NONE"),
					50) + spaces + dashes);
		}
	}
	...
	private Project createProject(Schedule unsolvedJobScheduling, ManufOrder manufOrder,
		...
		List<Integer> sortedPriorityList = new ArrayList<Integer>(
				new TreeSet<Integer>(priorityToProdProcessLineMap.keySet()));
		...
		long projectId = unsolvedJobScheduling.getProjectList().size() > 0
				? unsolvedJobScheduling.getProjectList().get(unsolvedJobScheduling.getProjectList().size() - 1).getId()
						+ 1
				: 0;
		...
		long sourceJobId = unsolvedJobScheduling.getJobList().size() > 0
				? unsolvedJobScheduling.getJobList().get(unsolvedJobScheduling.getJobList().size() - 1).getId() + 1
				: 0;
		...
		sourceAllocation.setSuccessorAllocationList(priorityToAllocationMap.get(sortedPriorityList.get(0)));
		...
		Job sinkJob = new Job();
		long sinkJobId = unsolvedJobScheduling.getJobList().size() > 0
				? unsolvedJobScheduling.getJobList().get(unsolvedJobScheduling.getJobList().size() - 1).getId() + 1
				: 0;
		...
		for (int priorityIdx = 0; priorityIdx < sortedPriorityList.size(); priorityIdx++) {
			...
			for (ProdProcessLine prodProcessLine : priorityToProdProcessLineMap.get(priority)) {
				// Job
				Job job = new Job();
				long jobId = unsolvedJobScheduling.getJobList().size() > 0
						? unsolvedJobScheduling.getJobList().get(unsolvedJobScheduling.getJobList().size() - 1).getId()
								+ 1
						: 0;
				job.setId(jobId);
				...
				long executionModeId = unsolvedJobScheduling.getExecutionModeList().size() > 0
						? unsolvedJobScheduling.getExecutionModeList()
								.get(unsolvedJobScheduling.getExecutionModeList().size() - 1).getId() + 1
						: 0;
				...
				if (prodProcessLine.getWorkCenter().getWorkCenterTypeSelect() != 1) {
					duration = (long) (prodProcessLine.getWorkCenter().getDurationPerCycle()
							* Math.ceil((float) manufOrder.getQty().intValue()
									/ prodProcessLine.getWorkCenter().getMaxCapacityPerCycle().intValue()));
				} else if (prodProcessLine.getWorkCenter().getWorkCenterTypeSelect() == 1) {
					duration = prodProcessLine.getWorkCenter().getProdHumanResourceList().get(0).getDuration()
				...
				long resourceRequirementId = unsolvedJobScheduling.getResourceRequirementList().size() > 0
						? unsolvedJobScheduling.getResourceRequirementList()
								.get(unsolvedJobScheduling.getResourceRequirementList().size() - 1).getId() + 1
						: 0;
				...
				List<Allocation> predecessorAllocationList = priorityIdx > 0
				...
				List<Allocation> successorAllocationList = priorityIdx < sortedPriorityList.size() - 1
						? priorityToAllocationMap.get(sortedPriorityList.get(priorityIdx + 1))
						: sinkAllocationList;
